...
- Includes weather condition icons mapped to Home Assistant's standards.
...
# Weather Service (weather entity)
...
- **Precipitation (mm)** (daily forecasts only)
...
3 Hour and daily weather forecasts are retrieved from the forecast1h, forecast3h and forecast24h sections of the ARSO.
...
##UPDATE: Since version 1.3.0 separate sensors for current weather data are created. By default only Temperature, Condition and Weather Phenomenon are *enabled* by default. If you want the other sensors as well you might want to enabled them.
